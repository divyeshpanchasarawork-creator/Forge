...
- All UI built from shared primitives: `Card`/`CardContent`/`CardHeader`, `Button` (+`buttonVariants` for links styled as buttons), `Badge`, `Input`, `SectionHeader` (h2 + icon + action), `HeroCard` (gradient hero), `Callout` (icon + tone), `EmptyState` (dashed, icon/title/description/action), `StatTile` (metric tile with tone), `KpiCard` (hover tooltip metric), `ProgressBar` (tone), `ReadinessRing`, `SignalChip` (score-breakdown pill tag with tooltip: `rounded-full bg-secondary px-2 py-0.5 text-tag` muted name with `+contribution` value in `text-primary`, subordinate to the title; `text-tag` = 10px type token), `ApiErrorState`, `LoadingSkeleton`, `TeachingEmptyState`, `AppBootScreen`
- Semantic tokens in `index.css` `@theme`: primary, success, warning, destructive, info, secondary, muted/muted-foreground, subtle-foreground, elevated, border, input, card, background; text sizes `text-micro/caption/section`; `shadow-cta`; `fade-in-up` animation; `.bg-dots` utility (theme-adaptive dot grid) — use as a `fixed inset-0 -z-10` full-canvas page layer on an opaque-free wrapper (body provides `bg-background`); never nest it inside an opaque `bg-background` element or it won't paint
...
- Every calibration run — applied, kept-weights, or skipped — appends a row to `calibration_runs` (`CalibrationRun`, V33); `/api/internal/engine-report` returns the last 30 as `recentRuns` and ProfilePage charts holdout-MSE trend from them
...
- `CandidatePoolService.rankForUser` caps stored RECOMMENDATION suggestions at half the pool so tag-derived candidates always contribute; `tagSlugForTopic` fuzzy-matches via TitleMatcher
